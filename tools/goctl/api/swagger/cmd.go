...
import (
	...
	"github.com/zeromicro/go-zero/tools/goctl/api/swagger/generate"
...
func SwaggerCommand(_ *cobra.Command, _ []string) error {

	if len(VarStringFilename) == 0 {
		VarStringFilename = "rest.swagger.json"
	}
	...
	p.Dir = dirAbs
	p.Style = VarStringStyle
	api, err := parser.Parse(p.ApiFilePath)
	if err != nil {
		return err
	}
	p.Api = api
	return generate.Do(VarStringFilename, VarStringHost, VarStringBasepath, VarStringSchemes, &p)
}
