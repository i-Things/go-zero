...
const (
	...
	wsFilename     = "wsRoutes"
	wsTemplate     = `// Code generated by goctl. DO NOT EDIT.
package handler
...
func genRoutes(dir, rootPkg string, cfg *config.Config, api *spec.ApiSpec, ws bool) error {
	var builder strings.Builder
	var wsBuilder strings.Builder
...
rest.WithPrefix("%s"),`, g.prefix)
		...
		if err := gt.Execute(&wsBuilder, map[string]string{
			"routes":    routes,
			"jwt":       strings.ReplaceAll(jwt, "rest.With", "ws.With"),
			"signature": strings.ReplaceAll(signature, "rest.With", "ws.With"),
			"prefix":    strings.ReplaceAll(prefix, "rest.With", "ws.With"),
			"timeout":   strings.ReplaceAll(timeout, "rest.With", "ws.With"),
			"maxBytes":  strings.ReplaceAll(maxBytes, "rest.With", "ws.With"),
		}); err != nil {
			return err
		}
	}
	...
	wsFilename, err := format.FileNamingFormat(cfg.NamingFormat, wsFilename)
	if err != nil {
		return err
	}
	...
	os.Remove(filename)
	wsFilename = wsFilename + ".go"
	filename = path.Join(dir, handlerDir, wsFilename)
	os.Remove(filename)
	...
	if ws {
		return genFile(fileGenConfig{
			...
			subdir:          handlerDir,
			filename:        wsFilename,
			templateName:    "wsTemplate",
			category:        category,
			templateFile:    wsTemplateFile,
			builtinTemplate: wsTemplate,
			data: map[string]any{
				"hasTimeout":      hasTimeout,
				"importPackages":  genRouteImports(rootPkg, api),
				"routesAdditions": strings.TrimSpace(wsBuilder.String()),
			},
		})
	}
	return nil
